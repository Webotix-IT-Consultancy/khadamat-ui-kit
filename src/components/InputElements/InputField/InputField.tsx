...
const InputField = React.forwardRef(({
  ...
  const renderIcon = (iconName: any, isError = false,size) => {
  ...
  const renderEyeIcon = (isVisible: boolean, isError = false,size) => {
  ...
  return (
    <div className="input-field">
      ...
      <div className={`relative input-wrapper focus-within:ring-4 focus-within:ring-primary-light focus-within:border-primary ${error ? 'input-wrapper-error' : ''}`}>
        ...
        {icon && <div className="input-icon-left">{renderIcon(icon, error,20)}</div>}
        <input
          type={inputType}
        ...
        {showPasswordToggle && (
          ...
          >
            {renderEyeIcon(showPassword, error, 20)}
          </button>
        )}
      </div>
